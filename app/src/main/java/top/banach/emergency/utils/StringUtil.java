...
public class StringUtil {
	...
	public static boolean isEmpty(String str) {
		...
		return null == str || "".equals(str) || str.length() == 0 || str.equals("null");
	}
	...
	/**
	 * 获取距离当前日期（date：如 ... -1 或 1...）天的时间
	 * 
	 * @param date
	 * @param format
	 * @return
	 */
	public static String getDateByNow(int date, String format) {
		try {
			Calendar c = Calendar.getInstance();
			c.add(Calendar.DATE, date);
			Date target = c.getTime();
			return getDate(target, format);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
